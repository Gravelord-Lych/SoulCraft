...
public enum MonsterViewBuff implements PlayerBuff {
    ...
    public void tick(PlayerEntity player, ServerWorld world) {
        for (LivingEntity entity : world.getEntitiesOfClass(LivingEntity.class, player.getBoundingBox().inflate(ModConstants.Exa.MONSTER_VIEW_RANGE), e -> e instanceof IMob)) {
            if (entity.distanceToSqr(player) > ModConstants.Exa.MONSTER_VIEW_RANGE * ModConstants.Exa.MONSTER_VIEW_RANGE) {
                continue;
            }
