...
public final class ReinforcementHelper {
    ...
    public static Map<Reinforcement, Integer> deserializeNBT(ListNBT listNBT) {
        ...
        for (int i = 0; i < listNBT.size(); i++) {
            ...
            if (location == null) {
                SoulCraft.LOGGER.warn(Reinforcements.REINFORCEMENT, "{} is not a valid ResourceLocation, skipped", name);
                continue;
            }
            ...
            if (reinforcement == null) {
                SoulCraft.LOGGER.warn(Reinforcements.REINFORCEMENT, "Reinforcement {} has unknown EntityType, skipped", location.getPath());
                continue;
            }
    ...
    public enum ApplicationStatus implements IIdentifiableEnum {
        OK(null),
        DUPLICATE(new TranslationTextComponent(SoulCraft.prefixMsg("reinforcement", "status.duplicate"))),
        INCOMPATIBLE(new TranslationTextComponent(SoulCraft.prefixMsg("reinforcement", "status.incompatible"))),
        LEVEL_TOO_HIGH(new TranslationTextComponent(SoulCraft.prefixMsg("reinforcement", "status.level_too_high"))),
        REINFORCED(new TranslationTextComponent(SoulCraft.prefixMsg("reinforcement", "status.reinforced"))),
        UNSUPPORTED(new TranslationTextComponent(SoulCraft.prefixMsg("reinforcement", "status.unsupported")));
