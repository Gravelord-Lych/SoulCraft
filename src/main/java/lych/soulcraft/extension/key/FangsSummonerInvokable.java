...
public enum FangsSummonerInvokable implements IExtraAbilityRelatedInvokable {
    ...
    public int getInvokeResult(ServerPlayerEntity player, int recentlyPressed) {
        double minY = player.getY() - ModConstants.Exa.FANGS_MAX_Y_OFFSET;
        double maxY = player.getY() + ModConstants.Exa.FANGS_MAX_Y_OFFSET;
        ...
        for (int i = 0; i < ModConstants.Exa.FANGS_SUMMONER_COUNT; ++i) {
            double spacing = ModConstants.Exa.FANGS_SPACING * (i + 1);
    ...
    private static boolean createFangs(ServerPlayerEntity player, Vector3d vectorToTarget, double minY, double maxY, double yRot, int i) {
        return EntityUtils.createFangs(player.getX() + vectorToTarget.x, player.getZ() + vectorToTarget.z, minY, maxY, (float) yRot, i + 1, player, player.getLevel(), fangs -> fangs.setDamage(ModConstants.Exa.FANGS_DAMAGE));
    }
