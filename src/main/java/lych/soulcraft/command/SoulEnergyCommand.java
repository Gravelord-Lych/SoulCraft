...
public final class SoulEnergyCommand {
    private static final SimpleCommandExceptionType ERROR_EMPTY = new SimpleCommandExceptionType(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.empty_stack")));
    private static final SimpleCommandExceptionType ERROR_SAME_STACK = new SimpleCommandExceptionType(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.same_stack")));
    private static final DynamicCommandExceptionType ERROR_STACK_NOT_FOUND = new DynamicCommandExceptionType(ordinal -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.stack_not_found"), ordinal));
    private static final Dynamic2CommandExceptionType ILLEGAL_VALUE = new Dynamic2CommandExceptionType((expected, provided) -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.illegal_value"), provided, expected));
...
//        SoulCraft.LOGGER.info(amount);
        ...
        if (amount == actualChanged.intValue()) {
            context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.changed_success" + (amount > 0 ? "_receive" : "_extract")), Math.abs(amount)), true);
        } else {
            if (amount >= 0 && actualChanged.intValue() >= 0) {
                context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.changed_half_success_receive"), amount, actualChanged.intValue()), true);
            } else if (amount <= 0 && actualChanged.intValue() <= 0){
                context.getSource().sendSuccess(
                        new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.changed_half_success_extract"), amount, actualChanged.intValue()), true);
            } else {
    ...
    private static int setEnergy(CommandContext<CommandSource> context, ItemStack stack) throws CommandSyntaxException {
        ...
        if (expectedNewValue < 0) {
            throw new IllegalStateException(String.format("ExpectedNewValue is negative, maybe %s is broken", SoulCraft.MOD_NAME));
        }
        ...
        context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.set_success"), expectedNewValue), true);
    ...
    private static int transferEnergy(CommandContext<CommandSource> context, ItemStack src, ItemStack target, int amount) throws CommandSyntaxException {
        ...
        if (amount == actualTransferValue) {
            context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.transferred_success"), amount), true);
        } else if (actualTransferValue >= 0) {
            context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "soulenergy.transferred_half_success"), amount, actualTransferValue), true);
        } else {
