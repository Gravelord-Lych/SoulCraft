...
import lych.soulcraft.command.argument.TieredBossArgument;
...
public final class ResetBossCommand {
    ...
    private static <T extends Entity> int clearTierOf(EntityType<? extends T> type, CommandContext<CommandSource> context, int tier) throws CommandSyntaxException {
        ...
        context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "resetboss.success"), new TranslationTextComponent(type.getDescriptionId()).getString(), tier, player.getDisplayName()), true);
