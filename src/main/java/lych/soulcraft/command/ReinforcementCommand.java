...
public final class ReinforcementCommand {
    private static final DynamicCommandExceptionType ERROR_NO_ITEM = new DynamicCommandExceptionType(player -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.no_item"), player));
    private static final DynamicCommandExceptionType ERROR_UNSUPPORTED_ITEM = new DynamicCommandExceptionType(stack -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.unsupported_item"), stack));
    private static final Dynamic2CommandExceptionType ERROR_LEVEL_TOO_HIGH = new Dynamic2CommandExceptionType((level, maxLevel) -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.level_too_high"), level, maxLevel));
    private static final DynamicCommandExceptionType ERROR_TOO_MANY_REINFORCEMENTS = new DynamicCommandExceptionType(stack -> new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.too_many"), stack));
    ...
    private static int removeAllReinforcements(CommandContext<CommandSource> context, ServerPlayerEntity player) throws CommandSyntaxException {
        ...
        context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.remove_all"), player.getDisplayName()), true);
    ...
    private static int addReinforcement(CommandContext<CommandSource> context, ServerPlayerEntity player, int level) throws CommandSyntaxException {
        ...
        if (ReinforcementHelper.containsReinforcement(stack, reinforcement)) {
            context.getSource().sendFailure(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.add_failed"), reinforcement.getType().getDescription(), player.getDisplayName()));
            return 0;
        }
    ...
    private static int removeReinforcement(CommandContext<CommandSource> context, ServerPlayerEntity player) throws CommandSyntaxException {
        ...
        if (ReinforcementHelper.removeReinforcement(stack, reinforcement)) {
            context.getSource().sendSuccess(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.remove_success"), reinforcement.getType().getDescription().copy().withStyle(reinforcement.getStyle()), player.getDisplayName()), true);
            return Command.SINGLE_SUCCESS;
        } else {
            context.getSource().sendFailure(new TranslationTextComponent(SoulCraft.prefixMsg("commands", "reinforcement.remove_failed"), reinforcement.getType().getDescription(), player.getDisplayName()));
            return 0;
        }
    }
}
