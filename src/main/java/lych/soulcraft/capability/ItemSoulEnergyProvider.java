...
public class ItemSoulEnergyProvider implements ICapabilityProvider, INBTSerializable<CompoundNBT> {
    ...
    private static class ItemSoulEnergyStorage implements ISoulEnergyStorage {
        private static final String SOUL_ENERGY = "SoulEnergy.Energy";
        ...
        protected void actuallyReceive(int energyReceived) {
            stack.get().getOrCreateTag().putInt(SOUL_ENERGY, stack.get().getOrCreateTag().getInt(SOUL_ENERGY) + energyReceived);
        }
        ...
        protected void actuallyExtract(int energyExtracted) {
            stack.get().getOrCreateTag().putInt(SOUL_ENERGY, stack.get().getOrCreateTag().getInt(SOUL_ENERGY) - energyExtracted);
        }
        ...
        public int getSoulEnergyStored() {
            if (stack.get().hasTag()) {
                int energy = Objects.requireNonNull(stack.get().getTag()).getInt(SOUL_ENERGY);
